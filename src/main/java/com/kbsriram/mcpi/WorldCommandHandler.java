...
public class WorldCommandHandler {
	public final static class GetBlock implements ICommandHandler {
		public String handle(Command cmd, WorldServer ws) throws Exception {
			...
			/*
			 * Block b = ws.getBlock(x, y, z); return
			 * String.valueOf(Block.getIdFromBlock(b));
			 */
			IBlockState bs = ws.getMinecraftServer().getEntityWorld().getBlockState(new BlockPos(x,y,z));
			return String.valueOf(Block.getIdFromBlock(bs.getBlock()));
		}
	}
	...
	public final static class GetHeight implements ICommandHandler {
		public String handle(Command cmd, WorldServer ws)
			...
			return String.valueOf(ws.getHeight(x, z) - info.getSpawnY());
		}
	}
	...
	public final static class SetBlock implements ICommandHandler {
		public String handle(Command cmd, WorldServer ws) throws Exception {
			...
			// 
			World world = ws.getMinecraftServer().getEntityWorld();
			Block blk = Block.getBlockById(bid);
			world.setBlockState(new BlockPos(x,y,z), blk.getDefaultState(), flags);
	...
	public final static class SetTileEntityHex implements ICommandHandler {
		public String handle(Command cmd, WorldServer ws) throws Exception {
			...
			tag.setInteger("z", z);
			/*
			 * TODO TileEntity te = TileEntity.createAndLoadEntity(tag); if (te
			 * == null) { return "no such tileentity available."; }
			 * ws.setTileEntity(x, y, z, te); te.updateContainingBlockInfo();
			...
			for (int x = x1; x <= x2; x++) {
				for (int z = z1; z <= z2; z++) {
					for (int y = y1; y <= y2; y++) {
						...
						World world = ws.getMinecraftServer().getEntityWorld();
						Block blk = Block.getBlockById(bid);
						world.setBlockState(new BlockPos(x,y,z), b.getDefaultState(), flags);
